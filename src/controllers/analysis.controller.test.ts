import { REQUIRED_FILES } from '@/constant'
import {
  createAnalysis,
...
describe('Analysis Controller', () => {
  ...
  describe('createAnalysis', () => {
    ...
    it.each`
      ...
      ${1} | ${'returns 200 with valid input'}        | ${{ type: 'CDI' }} | ${{ projectName: 'P', numberOfGroup: 2, groupPercentage: 20 }} | ${generateFiles()}      | ${{ data: { code: 200, message: 'Analysis OK' } }} | ${200}         | ${'Analysis OK'}
      ${2} | ${'returns 400 when type is missing'}    | ${{}}              | ${{}}                                                          | ${generateFiles()}      | ${undefined}                                       | ${400}         | ${'Query parameter "type" is required'}
      ${3} | ${'returns 400 when file is missing'}    | ${{ type: 'CDI' }} | ${{}}                                                          | ${generateFiles(false)} | ${undefined}                                       | ${400}         | ${'Missing file'}
      ${4} | ${'returns 500 when Flask returns null'} | ${{ type: 'CDI' }} | ${{ projectName: 'P' }}                                        | ${generateFiles()}      | ${{ data: null }}                                  | ${500}         | ${'Analysis failed'}
  ...
  describe('getQuestionAnalysis', () => {
    it.each`
      case | desc                                      | query                         | supabaseResult                                 | expectedStatus | expectedMessage
      ${1} | ${'returns 200 with question data'}       | ${{ questionId: 'q123' }}     | ${{ data: { id: 'q123', content: 'Q?' } }}     | ${200}         | ${'Specific question analysis retrieved'}
